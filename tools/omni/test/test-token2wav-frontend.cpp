...
static void test_onnx_output_shape_contracts_require_single_batch() {
    std::string error;
...
static void test_native_frontend_with_real_models(const char * ref_wav,
                                                   const char * speech_tokenizer_onnx,
                                                   const char * campplus_onnx,
                                                   const char * dump_dir = nullptr) {
    omni::flow::FrontendPromptBundle bundle;
    std::string error;
    const bool ok = omni::flow::Token2WavFrontend::prepare_prompt_bundle(
        ref_wav, speech_tokenizer_onnx, campplus_onnx, bundle, &error, 1);
    if (!ok) {
...
int main(int argc, char ** argv) {
    ...
    test_onnx_output_shape_contracts_require_single_batch();
