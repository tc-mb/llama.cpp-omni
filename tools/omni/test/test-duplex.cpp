...
static void duplex_test_case(struct omni_context * ctx_omni,
    ...
    const char * repeat_img_env = std::getenv("OMNI_REPEAT_IMAGE");
    bool repeat_image = (repeat_img_env && std::string(repeat_img_env) == "1");
    for (int il = 0; il < cnt; ++il) {
        ...
        if (file_exists(img)) {
            ...
            frames[il].img_fname = repeat_image ? first_img : img;
        }
    ...
    if (repeat_image) {
        printf("  [repeat-image] all frames use: %s\n", first_img.c_str());
    }
