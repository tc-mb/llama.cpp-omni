...
struct audition_ctx {
    ...
    audition_ctx(audition_context_params & audition_params) {
        ...
        if (backend) {
            ...
            const char * apm_legacy_graph = std::getenv("OMNI_APM_LEGACY_CANN_GRAPH");
            const char * apm_eager = std::getenv("OMNI_APM_CANN_EAGER");
            const bool legacy_graph = apm_legacy_graph
                && apm_legacy_graph[0] == '1' && apm_legacy_graph[1] == '\0';
            const bool eager_requested = apm_eager
                ? apm_eager[0] == '1' && apm_eager[1] == '\0'
                : true;
            if (!legacy_graph && eager_requested) {
                ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(
                    ggml_backend_get_device(backend));
                using cann_set_disable_graph_t = void (*)(ggml_backend_t, bool);
                auto setter = reinterpret_cast<cann_set_disable_graph_t>(
                    ggml_backend_reg_get_proc_address(
                        reg, "ggml_backend_cann_set_disable_graph"));
                if (setter) {
                    setter(backend, true);
                    LOG_INF("%s: APM-local CANN eager mode enabled\n", __func__);
                } else {
                    LOG_WRN("%s: APM eager requested, but backend has no CANN graph bypass\n",
                            __func__);
                }
            }
...
struct audition_graph {
    ...
    ggml_cgraph * build_whisper() {
        ...
        // MiniCPM-o encodes audio with a 1-second chunk mask.  Each query may
        // attend to all preceding chunks and every token in its own 50-token
        // chunk, but not to future chunks.  This is also used for offline
        // reference audio and is therefore different from unrestricted
        // bidirectional Whisper attention for clips longer than one second.
...
static bool log_mel_spectrogram(
    ...
    mel.n_mel = n_mel;
    // The official processor first pads each waveform to the 30-second Whisper
    // window, computes the spectrogram, removes only that global final frame,
    // then crops by attention_mask.sum() = ceil(original_samples / hop).
    // Reproduce the same cropped result without allocating the 30-second tail.
    // Using floor here drops the final partially filled frame for non-aligned
    // reference audio (62464 samples: 390 instead of the official 391).
...
bool audition_audio_preprocess(
    ...
    // Keep the original offline length.  The official MiniCPM-o processor does
    // not round reference audio up to a 100 ms boundary; doing so changes both
    // the final mel frames and the number of LLM audio embeddings (for example
    // 3.904 s becomes 40 tokens here instead of the official 39).  Only retain
    // a minimum-length guard for exceptionally short input.
