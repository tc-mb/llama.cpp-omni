...
#if defined(OMNI_T2W_ENABLE_NATIVE_FRONTEND)
...
static bool ort_status_ok(const OrtApi * api,
                          OrtStatus *  status,
                          const char * stage,
                          std::string * error) {
    if (!status) {
    ...
    const char * message = api && api->GetErrorMessage ? api->GetErrorMessage(status) : nullptr;
    std::string detail = message ? message : "unknown ONNX Runtime error";
    if (api && api->ReleaseStatus) {
        api->ReleaseStatus(status);
    }
    return set_error(error, std::string(stage) + ": " + detail);
}

class OrtModelRunner {
  public:
    OrtModelRunner() = default;
    ~OrtModelRunner() { reset(); }

    OrtModelRunner(const OrtModelRunner &)             = delete;
    OrtModelRunner & operator=(const OrtModelRunner &) = delete;

    bool init(const std::string & model_path, int num_threads, std::string * error) {
        reset();
        const std::filesystem::path native_model_path = std::filesystem::u8path(model_path);
        std::ifstream model_file(native_model_path, std::ios::binary);
        if (!model_file.good()) {
            return set_error(error, "model file is not readable: " + model_path);
        }

        const OrtApiBase * base = OrtGetApiBase();
        if (!base || !base->GetApi) {
            return set_error(error, "OrtGetApiBase returned no API base");
        }
        api_ = base->GetApi(ORT_API_VERSION);
        if (!api_) {
            const char * version = base->GetVersionString ? base->GetVersionString() : nullptr;
            return set_error(error,
                             "ONNX Runtime API version " + std::to_string(ORT_API_VERSION) +
                                 " is unavailable (runtime=" + (version ? version : "unknown") + ")");
        }

        if (!ort_status_ok(api_, api_->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "omni-token2wav-frontend", &env_),
                           "CreateEnv", error) ||
            !ort_status_ok(api_, api_->CreateSessionOptions(&options_), "CreateSessionOptions", error) ||
            !ort_status_ok(api_, api_->SetSessionGraphOptimizationLevel(options_, ORT_ENABLE_ALL),
                           "SetSessionGraphOptimizationLevel", error) ||
            !ort_status_ok(api_, api_->SetIntraOpNumThreads(options_, std::max(1, num_threads)),
                           "SetIntraOpNumThreads", error)) {
            reset();
            return false;
        }

        if (!ort_status_ok(api_, api_->CreateSession(env_, native_model_path.c_str(), options_, &session_),
                           "CreateSession", error) ||
            !ort_status_ok(api_, api_->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &memory_info_),
                           "CreateCpuMemoryInfo", error)) {
            reset();
            return false;
        }
        return true;
    ...
    bool input_name(size_t index, std::string & out, std::string * error) const {
        return get_name(true, index, out, error);
    }

    bool output_name(size_t index, std::string & out, std::string * error) const {
        return get_name(false, index, out, error);
    }

    bool run(const std::vector<const char *> & input_names,
             const std::vector<OrtValue *> &  inputs,
             const std::vector<const char *> & output_names,
             std::vector<OrtValue *> &        outputs,
             std::string *                    error) const {
        outputs.assign(output_names.size(), nullptr);
        const OrtStatus * status = api_->Run(
            session_, nullptr, input_names.data(), inputs.data(), inputs.size(), output_names.data(),
            output_names.size(), outputs.data());
        return ort_status_ok(api_, const_cast<OrtStatus *>(status), "Run", error);
    }

  private:
    bool get_name(bool input, size_t index, std::string & out, std::string * error) const {
        OrtAllocator * allocator = nullptr;
        if (!ort_status_ok(api_, api_->GetAllocatorWithDefaultOptions(&allocator),
                           "GetAllocatorWithDefaultOptions", error) ||
            !allocator) {
            return false;
        }
        ...
        OrtStatus * status = input
                                 ? api_->SessionGetInputName(session_, index, allocator, &name)
                                 : api_->SessionGetOutputName(session_, index, allocator, &name);
        if (!ort_status_ok(api_, status, input ? "SessionGetInputName" : "SessionGetOutputName", error)) {
            return false;
        }
        if (!name) {
            return set_error(error, "ONNX Runtime returned an empty tensor name");
        }
        out = name;
...
static bool ort_get_output_shape(const OrtApi *              api,
                                 const OrtValue *             value,
                                 std::vector<int64_t> &       shape,
                                 ONNXTensorElementDataType & type,
                                 std::string *               error) {
    OrtTensorTypeAndShapeInfo * info = nullptr;
    if (!ort_status_ok(api, api->GetTensorTypeAndShape(value, &info), "GetTensorTypeAndShape", error) || !info) {
        return false;
    }

    size_t dimension_count = 0;
    bool ok = ort_status_ok(api, api->GetDimensionsCount(info, &dimension_count), "GetDimensionsCount", error);
    if (ok) {
        shape.resize(dimension_count);
        ok = ort_status_ok(api, api->GetDimensions(info, shape.data(), shape.size()), "GetDimensions", error);
    }
    if (ok) {
        ok = ort_status_ok(api, api->GetTensorElementType(info, &type), "GetTensorElementType", error);
    }
    api->ReleaseTensorTypeAndShapeInfo(info);
    return ok;
}

static bool tensor_element_count(const std::vector<int64_t> & shape, size_t & count, std::string * error) {
    count = 1;
    for (const int64_t dim : shape) {
        if (dim <= 0) {
            return set_error(error, "ONNX Runtime returned a non-positive output dimension");
        }
        const size_t size_dim = static_cast<size_t>(dim);
        if (count > std::numeric_limits<size_t>::max() / size_dim) {
            return set_error(error, "ONNX Runtime output tensor is too large");
        }
        count *= size_dim;
    }
    return true;
...
static bool run_speech_tokenizer(const AudioFeatures & features,
    ...
    if (features.channels != 128 || features.frames <= 0 ||
        features.values.size() != static_cast<size_t>(features.channels) * features.frames) {
        return set_error(error, "speech tokenizer features must have shape [128, T]");
    }

    OrtModelRunner runner;
    if (!runner.init(model_path, num_threads, error)) {
        return false;
    }

    std::string feats_name;
    std::string length_name;
    std::string output_name;
    if (!runner.input_name(0, feats_name, error) ||
        !runner.input_name(1, length_name, error) ||
        !runner.output_name(0, output_name, error)) {
        return false;
    }
    if (feats_name != "feats" || length_name != "feats_length") {
        return set_error(error,
                         "speech tokenizer input contract mismatch: expected feats and feats_length, got " +
                             feats_name + " and " + length_name);
    }

    const int64_t feature_shape[] = { 1, features.channels, features.frames };
    const int64_t length_shape[]  = { 1 };
    const int32_t feature_length[] = { features.frames };
    OrtValue *   feature_value = nullptr;
    OrtValue *   length_value  = nullptr;
    const OrtApi * api         = runner.api();
    if (!ort_status_ok(api,
                       api->CreateTensorWithDataAsOrtValue(
                           runner.memory_info(), const_cast<float *>(features.values.data()),
                           features.values.size() * sizeof(float), feature_shape, 3,
                           ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &feature_value),
                       "CreateTensor(feats)", error) ||
        !ort_status_ok(api,
                       api->CreateTensorWithDataAsOrtValue(
                           runner.memory_info(), const_cast<int32_t *>(feature_length),
                           sizeof(feature_length), length_shape, 1,
                           ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32, &length_value),
                       "CreateTensor(feats_length)", error)) {
        if (feature_value) {
            api->ReleaseValue(feature_value);
        }
        if (length_value) {
    ...
    const std::vector<const char *> input_names = { feats_name.c_str(), length_name.c_str() };
    const std::vector<OrtValue *>  inputs       = { feature_value, length_value };
    const std::vector<const char *> output_names = { output_name.c_str() };
    std::vector<OrtValue *> outputs;
    const bool run_ok = runner.run(input_names, inputs, output_names, outputs, error);
    api->ReleaseValue(feature_value);
    api->ReleaseValue(length_value);
    if (!run_ok || outputs.empty() || !outputs[0]) {
    ...
    bool ok = ort_get_output_shape(api, outputs[0], shape, type, error);
    size_t count = 0;
    if (ok) {
        ok = Token2WavFrontend::validate_speech_tokenizer_output_shape(shape, error);
    }
    if (ok) {
        ok = tensor_element_count(shape, count, error);
    }
    void * output_data = nullptr;
    if (ok) {
        ok = ort_status_ok(api, api->GetTensorMutableData(outputs[0], &output_data),
                           "GetTensorMutableData(indices)", error);
    }
    if (ok && type != ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32 &&
        type != ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64) {
        ok = set_error(error, "speech tokenizer output must be int32 or int64");
    }
    if (ok && count == 0) {
        ok = set_error(error, "speech tokenizer returned no tokens");
    }
    if (ok) {
        tokens.resize(count);
        if (type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
            const auto * values = static_cast<const int32_t *>(output_data);
            std::copy(values, values + count, tokens.begin());
        } else {
            const auto * values = static_cast<const int64_t *>(output_data);
            for (size_t i = 0; i < count; ++i) {
                if (values[i] < std::numeric_limits<int32_t>::min() ||
                    values[i] > std::numeric_limits<int32_t>::max()) {
                    ok = set_error(error, "speech tokenizer returned a token outside int32 range");
                    break;
                }
                tokens[i] = static_cast<int32_t>(values[i]);
            }
        }
    }
    api->ReleaseValue(outputs[0]);
...
static bool run_campplus(const AudioFeatures & features,
                         const std::string & model_path,
                         int                    num_threads,
    ...
    if (features.channels != 80 || features.frames <= 0 ||
        features.values.size() != static_cast<size_t>(features.channels) * features.frames) {
        return set_error(error, "CampPlus features must have shape [T, 80]");
    }
    ...
    std::string output_name;
    if (!runner.input_name(0, input_name, error) ||
        !runner.output_name(0, output_name, error)) {
        return false;
    }

    const int64_t feature_shape[] = { 1, features.frames, features.channels };
    OrtValue *   feature_value = nullptr;
    const OrtApi * api         = runner.api();
    if (!ort_status_ok(api,
                       api->CreateTensorWithDataAsOrtValue(
                           ...
                           features.values.size() * sizeof(float), feature_shape, 3,
                           ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &feature_value),
                       "CreateTensor(CampPlus)", error)) {
        return false;
    }

    const std::vector<const char *> input_names = { input_name.c_str() };
    const std::vector<OrtValue *>  inputs       = { feature_value };
    const std::vector<const char *> output_names = { output_name.c_str() };
    std::vector<OrtValue *> outputs;
    const bool run_ok = runner.run(input_names, inputs, output_names, outputs, error);
    api->ReleaseValue(feature_value);
    ...
    size_t count = 0;
    if (ok) {
        ok = Token2WavFrontend::validate_campplus_output_shape(shape, error);
    }
    if (ok) {
        ok = tensor_element_count(shape, count, error);
    }
    if (ok && (type != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT || count != 192)) {
        ok = set_error(error, "CampPlus output must be float32 with 192 elements");
    }
    void * output_data = nullptr;
    if (ok) {
        ok = ort_status_ok(api, api->GetTensorMutableData(outputs[0], &output_data),
                           "GetTensorMutableData(CampPlus)", error);
    }
    if (ok) {
        const auto * values = static_cast<const float *>(output_data);
        speaker_embedding.assign(values, values + count);
        for (const float value : speaker_embedding) {
            if (!std::isfinite(value)) {
                ok = set_error(error, "CampPlus returned a non-finite speaker embedding");
                break;
            }
        }
    }
...
bool Token2WavFrontend::prepare_prompt_bundle(const std::string & ref_wav_path,
...
#if !defined(OMNI_T2W_ENABLE_NATIVE_FRONTEND)
    (void) ref_wav_path;
    (void) speech_tokenizer_onnx;
    (void) campplus_onnx;
    (void) num_threads;
    return set_error(error, "native Token2Wav frontend is disabled at build time");
#else
    AudioBuffer input;
    ...
    if (!run_speech_tokenizer(speech_features, speech_tokenizer_onnx, num_threads, speech_tokens, error)) {
        return false;
    }
    ...
    std::vector<float> speaker_embedding;
    if (!run_campplus(campplus_features, campplus_onnx, num_threads, speaker_embedding, error)) {
        return false;
    }
